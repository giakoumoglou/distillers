...
from datasets import get_imagenet_dataloader,  get_dataloader_sample, get_dali_data_loader

from utilities import train_distill as train, validate_vanilla, validate_distill
from utilities import save_dict_to_json, reduce_tensor, adjust_learning_rate

from distillers import DistillKL, HintLoss, Attention, Similarity, VIDLoss, SemCKDLoss
from distillers import CRDLoss, RRDLoss, DCDLoss
...
def parse_option():
    ...
    parser = argparse.ArgumentParser('Arguments for training student network')
    ...
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'hint', 'attention', 'similarity', 'vid', 'crd', 'semckd','srrl', 'simkd', 'rrd',])
    ...
    parser.add_argument('--dist-url', default='tcp://127.0.0.1:23451', type=str, help='url used to set up distributed training')
    ...
    # set paths to save model
    model_name_template = split_symbol.join(['S', '{}_T', '{}_{}_{}_r', '{}_a', '{}_b', '{}_{}'])
    opt.model_name = model_name_template.format(opt.model_s, opt.model_t, opt.dataset, opt.distill, opt.cls, opt.div, opt.beta, opt.trial)
    ...
    if torch.cuda.is_available():
        ...
        if opt.multiprocessing_distributed:
            if opt.gpu is not None:
                ...
                print('Multiprocessing distributed must be with a specifiec gpu id')
        else:
            criterion_list.cuda()
            module_list.cuda()
